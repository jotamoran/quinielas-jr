...
<template>
  <main class="page-shell max-w-6xl">
    ...
    <section v-if="tab === 'listado'" class="space-y-4">
      <div class="grid gap-3 rounded-2xl bg-white p-4 shadow-sm sm:grid-cols-[1fr_180px]"><input v-model="search" aria-label="Buscar quinielas" class="min-h-11 rounded-xl border-gray-300" placeholder="Buscar por entrada, persona o jornada" /><select v-model="filtroEstatus" aria-label="Filtrar por estatus" class="min-h-11 rounded-xl border-gray-300"><option value="todos">Todos los estatus</option><option v-for="item in estados" :key="item.value" :value="item.value">{{ item.label }}</option></select></div>
    ...
    <section v-else class="space-y-5">
      ...
      <button v-if="partidos.length" @click="registrar" :disabled="!completas || !alias.trim() || loading" class="mt-4 w-full rounded-xl bg-quiniela-verde py-3 font-bold text-white shadow-xl disabled:opacity-50 sm:sticky sm:bottom-3">{{ loading ? 'Registrando…' : `${Object.keys(pronosticos).length} de ${partidosActivos.length} · Registrar quiniela` }}</button>
    </section>
  </main>
</template>
