...
async function cerrar(id) {
  ...
  try {
    ...
    if (resultado.value.avisos?.length) {
      ...
      await nextTick();
      resumen.value?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    } else {
...
<template>
  <main class="page-shell max-w-3xl">
    ...
    <section v-if="resultado" ref="resumen" class="scroll-mt-24 rounded-2xl border border-green-200 bg-white p-5 shadow-sm">
