...
<template>
  <label class="relative block text-sm font-semibold">
    ...
    <ul v-if="options.length" role="listbox" class="absolute z-30 mt-1 max-h-52 w-full overflow-auto rounded-xl border bg-white p-1 shadow-xl">
      <li v-for="(team, index) in options" :key="team.id" :id="`equipo-opcion-${index}`" role="option" :aria-selected="activo === index"><button type="button" @click="select(team)" class="flex w-full items-center gap-3 rounded-lg p-2 text-left hover:bg-green-50" :class="activo === index ? 'bg-green-50' : ''"><img v-if="team.logo" :src="team.logo" alt="" loading="lazy" class="h-8 w-8 object-contain" /><span>{{ team.name }}</span></button></li>
    </ul>
