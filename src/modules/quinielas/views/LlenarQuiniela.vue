...
const cargandoJornada = ref(true);
...
const hayCambiosPendientes = computed(() => paso.value !== 'confirmacion' && (Object.values(pronosticos.value).some(Boolean) || alias.value.trim() !== 'Entrada 1'));

async function confirmarSalida() {
  if (!hayCambiosPendientes.value || procesando.value) return true;
  return confirmarAccion({ title: 'Salir de la quiniela', text: 'Perderás los pronósticos que todavía no has registrado.', confirmText: 'Salir', danger: true });
}

function advertirAntesDeSalir(evento) {
  if (!hayCambiosPendientes.value || procesando.value) return;
...
async function confirmarPago({ metodo, archivo, codigoCupon, cantidad = 1 }) {
  ...
  try {
    ...
    if (comprobanteUrl) {
      try { await eliminarComprobante(comprobanteUrl); } catch { }
    }
...
onMounted(async () => { await cargar(); actualizarTiempo(); intervalo = setInterval(actualizarTiempo, 1000); window.addEventListener('beforeunload', advertirAntesDeSalir); });
...
<template>
  <main class="mx-auto max-w-3xl space-y-5 px-4 py-5 sm:px-6 lg:px-8">
    ...
    <div v-else-if="cargandoJornada" class="rounded-2xl border border-dashed bg-white p-8 text-center text-gray-500" role="status">Cargando jornada…</div>
    <div v-else-if="!jornada" class="rounded-2xl border border-dashed bg-white p-8 text-center text-gray-500">No hay una jornada activa en este momento.</div>
    ...
    <template v-else>
      ...
      <section v-if="paso === 'pronosticos'" class="space-y-4">
        ...
        <div v-if="urgencia !== 'normal' && !bloqueado" role="status" class="rounded-2xl border border-orange-200 bg-orange-50 p-4 text-sm font-semibold text-orange-800">El cierre está próximo. Revisa tus pronósticos y confirma tu entrada antes de {{ formatearFecha(jornada.fecha_cierre, { timeStyle: 'short' }) }} · CDMX.</div>
